...
![Eps Win1](63_eps-w_1.png)

Hit Ctrl+S and save the key in a folder on your computer (preferably where you download files to).
To verify the signature, we need Gpg4win. If it’s not already installed, go to <a href="https://www.gpg4win.org" target="_blank">
https://www.gpg4win.org</a>
 and install the latest release for Windows. We need to start a new terminal. Open “Powershell” (search for it or use Win+R, type powershell and hit enter).
...
Hw_wallet1 = multisig_wallet = xpub kg4QUp5XpUdNf2uGXvQmnD4zcofZ1MN6Fo8PjqQ…
...
The best solution is to use a strong (many random characters) “rpcuser” and a “rpcpassword” in Bitcoin Core (not the default).  You need to add this to config.cfg as well. Uncomment (remove “#”) the two lines “rpc_user” and “rpc_password” and add your information. 

To use a rpcuser and rpcpassword you need to add this to your bitcoin.conf file. If you are unsure if you’ve set this or not, look in the default location (C:\Users\User1\AppData\Roaming\Bitcoin\bitcoin.conf). If you’ve moved the installation, Bitcoin Core will try to use the .conf file in the new location. Edit the file at that location. The file also needs to contain the line server=1 (to accept connection from other programs). If you don’t have a bitcoin.conf file, simple create a .txt file add the information, save and rename the file to bitcoin.conf (make sure to change the extension from .txt to conf). So the file should at least contain the following
...
*Note*. “Py” is the Windows Python Launcher and can in some situation work when “python” doesn’t. But try to use “python” and if only “py” works change python to py in the following commands.
...
Rename the new text document to “Electrum-Personal-Server.bat” (make sure to change the .txt to .bat). You’ll get a warning about changing the file name extension, select Yes.

Run Electrum-Personal-Server.bat by double clicking. Electrum Server should now start and import addresses as watch only addresses in your Bitcoin Core node. Wait for the importing to finish. This will detect new transactions to addresses derived from your master public keys. 

###Troubleshooting. 
If the terminal (cmd.exe) starts and quickly exits the server isn’t starting properly. Go to the folder where Electrum-Personal-Server.bat, right click and select edit.

Add

Pause to a new line in the file. So, the content is something like:

C:\Users\User1\AppData\Roaming\Python\Python37\Scripts\electrum-personal-server.exe C:\Users\User1\Downloads\electrum-personal-server-eps-v0.1.6\config.cfg
pause 

This should leave cmd.exe open to read any error messages. If you get a error message like

WARNING:2019-02-27 09:32:22,102: Unable to find .cookie file, try setting `datadir` config
You need to set a rpc_user and a rpc_password.
...
###Start the server
Once the importing is done Electrum Server will exit. If you want to import old transactions, you need to rescan the Bitcoin blockchain. You can do this by making a copy of Electrum-Personal-Server.bat rename the copy to Electrum-Personal-Server-Rescan.bat , right click on the new file and select edit. Change the electrum-personal-server.exe to electrum-personal-server-rescan.exe. So, the line is something like:
...
If you want to import old addresses run Electrum-Personal-Server-Rescan.bat and enter a date (in the format DD/MM/YYYY) from where you want to start importing addresses (the further back, the longer time it’ll take) and hit enter. You will get a suggestion of a block height to start from. Enter y and hit enter. Wait for the rescanning to finish (the server will exit once finished).
...
That means that the server can’t connect to your Bitcoin Core full node. This is likely an issue with your conf-file (either in the server or in Bitcoin Core. This is a copy of my config.cfg-file that works on Windows 10. I use a rpcuser and rpcpassword in my Bitcoin config file (I’ve removed all comments “#” in this file for readability, can be a good idea to keep those)
...
and my Bitcoin.conf file (you can have other settings as well, this is what’s important for Electrum personal server)
...
Try another port for electrum server by changing the following line in the config.cfg file like
[electrum-server]
host = 127.0.0.1 
port = 8000
...
If cmd.exe closes without the server starting, edit Electrum-Personal-Server.bat (right click>Edit) and add pause on a new line. Save, close and run againg.
You could also check the log file that’s located in C:\Users\User1\AppData\Local\Temp\electrumpersonalserver.log
...
![Eps Win4](63_eps-w_4.png)

If you change port 50002 in config.cfg make sure to change to the same port here. Close this dialaog once finished 

The best way to do it is to make a shortcut that tells Electrum to only listen to our server. In that case you don’t risk connecting to a public server by mistake. If you don’t have a shortcut to Electrum on your desktop or in a folder. Navigate to the folder where Electrum is located, standard is C:\Program Files (x86)\Electrum. Right click on the .exe file, for example electrum-3.1.3.exe. Select Create Shortcut. You’ll get the following message:

![Eps Win5](63_eps-w_5.png)

Select Yes

Navigate to the desktop and right click on the shortcut. Select properties. In “Target” add the following to the end of the line --oneserver --server localhost:50002:s
