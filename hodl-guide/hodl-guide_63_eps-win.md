...
Hit `Ctrl+S` and save the key in a folder on your computer.
...
We need to start a new terminal. Open “PowerShell” (search for it or use Win+R, type powershell and click "OK").
...
It’s now imported (keep PowerShell open). In your web browser, navigate to the [release page](https://github.com/chris-belcher/electrum-personal-server/releases) and download the latest `Source code (.zip)` and the corresponding `.asc` file. For example:
...
You should now have the two scripts, `electrum-personal-server.exe` and `electrum-personal-server-rescan.exe` in a Python folder in the `%Appdata` location.
...
Change "python37" if using another version. If it doesn't work try navigating to the folder manually, it should be located at (AppData is usually a hidden folder):

`C:\Users\User1\AppData\Roaming\Python\Python37\Scripts`

Once in the folder, copy the full path to the file `electrum-personal-server.exe`.
...
Run `Electrum-Personal-Server.bat` by double clicking on it. Electrum Server should start and import addresses from the master public keys you defined in config.cfg as watch only addresses in your Bitcoin Core node. Wait for the importing to finish.

### Troubleshooting

If the terminal `cmd.exe` starts and quickly exits the server isn’t starting properly. Go to the file Electrum-Personal-Server.bat, right click on it and select edit. Add `Pause` to a new line in the file. So, the content is something like:
...
You might need to set a `rpc_user` and a `rpc_password` or change `datadir` to the correct path (don't use `#` in your password).
...
If you don't rescan and open an old wallet, the balance will show 0 as it doesn't know about any old transactions.

Once the rescanning is done, or if you use a new wallet. Run Electrum-Personal-Server.bat again. This will start the server. Wait for this message:
